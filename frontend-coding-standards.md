...
- `<div>` 里可以包含 `<p>` ，但是 `<p>` 里不允许包含 `<div>` 等块级元素；
...
- `<input>` 和 `<button>` 必须指明默认的 type，`<form>` 必须有默认的 method，这样可避免在不同浏览器下产生行为上的差异。
- 为了方便，建议 `<form>` 须加 action 属性，`<input>` 须加 name 和 id 属性，并且最好有对应的 `<label>` 标签，`<option>` 须加 value 属性，`<a>` 须加 href 属性，并在需要的时候为其赋值。
- 为增强语义化，按钮用 `<button>` 而不用 `<input>`。
...
标签应用在其适用的场合。每个标签都有特定的意义，不应把 `<div>` 当成万能标签使用。HTML5 新增的标签不是摆设，请以合理的方式加以利用。
